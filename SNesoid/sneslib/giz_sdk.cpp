...
#include <unistd.h>
#include <stdio.h>
#include <w32api/GizSdk/Framework.h>
#include <w32api/GizSdk/ddraw.h>
#include <w32api/GizSdk/Framework2D.h>
#include <w32api/GizSdk/gles.h>
#include <w32api/GizSdk/FrameworkGL.h>
...
//Temp framebuffers
unsigned short frameBufferMemory1[320*500]; //Oversize buffer to let allow centering of Snes output
...
void gp_drawString (int x,int y,int len,char *buffer,unsigned short color,void *framebuffer) 
{
	int l,base=0;
		...
		for (l=0;l<len;l++) 
		{
			...
			{
				set_char8x8_16bpp (x+base,y,buffer[l],color,(unsigned short*)framebuffer);
			}
			base+=8;
		}
}
...
unsigned int gp_getButton(unsigned char enable_diagnals)
{
	...
	return Framework_PollGetButtons();
}
...
void gp_initGraphics(unsigned short bpp, int flip, int applyMmuHack)
{
	...
	GizPrimaryFrameBuffer=(void*)Framework2D_LockBuffer(1);
		
	framebuffer8[0]=(unsigned char*)&frameBufferMemory1[320*5];
	framebuffer8[1]=(unsigned char*)&frameBufferMemory1[320*5];
	framebuffer8[2]=(unsigned char*)&frameBufferMemory1[320*5];
	framebuffer8[3]=(unsigned char*)&frameBufferMemory1[320*5];
	
	framebuffer16[0]=&frameBufferMemory1[320*5];
	framebuffer16[1]=&frameBufferMemory1[320*5];
	framebuffer16[2]=&frameBufferMemory1[320*5];
	framebuffer16[3]=&frameBufferMemory1[320*5];
...
void gp_setFramebuffer(int flip, int sync)
{
	if (bppMode==16)
	{
		...
		BlitBufferToScreen((void*)(unsigned short*)framebuffer16[flip],(void*)GizPrimaryFrameBuffer);
